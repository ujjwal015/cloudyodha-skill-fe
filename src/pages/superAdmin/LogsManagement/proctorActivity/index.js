...
import { LOGS_MANAGEMENT_PROCTOR_ACTIVITY_LOGS_CANDIDATE_BY_BATCH_LIST_PATH } from "../../../../config/constants/routePathConstants/superAdmin";
...
const BatchList = () => {
  ...
  const handleExport = () => {
    ...
    saveAs(blob, "Proctor List.xlsx");
  };

  const handleNavigateCandidateLogs = (id) => {
    navigate(
      `${LOGS_MANAGEMENT_PROCTOR_ACTIVITY_LOGS_CANDIDATE_BY_BATCH_LIST_PATH}/${id}`
    );
  };
                      ...
                      <td>
                        <div className="assesment-stats-btn">
                          <button
                            ...
                            onClick={() =>
                              handleNavigateCandidateLogs(item._id)
                            }
                          >
                            <span>View Logs</span>
                          </button>
...
const getColumns = (isRolePermission) => {
  let columns = [
    ...
    { name: "recordingProctoring", label: "REC.PROCTORING", sorting: false },
  ];
