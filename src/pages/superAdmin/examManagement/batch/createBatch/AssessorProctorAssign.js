...
const AssessorProctorAssign = (props) => {
  ...
  const { proctorList = [], assessorList = [] } = useSelector(examManagementSelector);
  ...
  const handleChange = (event) => {
    ...
    // If assignAssessorProctor is changed, clear related field errors
    if (name === "assignAssessorProctor") {
      setErrors((prevErrors) => {
        const { accessorId, proctorId, assessorFeePerCandidate, ...restErrors } = prevErrors;
        return restErrors;
      });
      return;
    }
    ...
    if (name === "proctorId") {
      // Clear any existing errors for proctorId
      setErrors((prevErrors) => {
        const { proctorId, ...restErrors } = prevErrors;
        return restErrors;
      });
      return;
    }

    // Special validation for assessorFeePerCandidate - only validate if assignAssessorProctor is true
    if (name === "assessorFeePerCandidate") {
    ...
    // Validate the field with assignAssessorProctor context.
    const fieldError = formValues?.assignAssessorProctor 
      ? validateField(name, value, { assignAssessorProctor: formValues?.assignAssessorProctor })
      : validateField(name, value);
  ...
  const changeHandler = (e) => {
    ...
    // Special handling for assessor field - only validate if assignAssessorProctor is true
    if (name === "accessorId") {
      const fieldError = formValues?.assignAssessorProctor 
        ? validateField(name, value, { assignAssessorProctor: formValues?.assignAssessorProctor })
        : validateField(name, value);
      if (fieldError) {
        ...
        setErrors((prevErrors) => {
          const { accessorId, ...restErrors } = prevErrors;
          return restErrors;
        });
      }
      return;
                ...
                <div
                  ...
                  <div style={{ width: "100%" }}>
                    <RadioButton
                      ...
                      value={formValues?.assignAssessorProctor}
