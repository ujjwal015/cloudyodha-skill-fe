...
useEffect(() => {
  if (jobroleId && job?.level) {
    dispatch(
      getVersionOfJobRoleSelectedApi(jobroleId, job?.level, setVersionList)
    );
  }
}, [jobroleId]);
...
console.log(`DeletedData--${index}`,levelList,"--", versionList)
  ...
  useEffect(() => {
    if (job?.jobRoleId && job?.level && job?.version) {
      dispatch(
        getVersionOfJobRoleSelectedApi(
          job.jobRoleId,
          job.level,
    ...
    <tr key={index}>
      ...
      <td style={{ ...styles.tableCell }}>
        <SelectInput
          ...
          // options={versionList}
          options={editable ? versionList : [{ label: job?.version, value: job?.version }]}
          error={errors?.[`${job?.jobRoleId}/version`]}
          mandatory
          // disabled={jobRoleLevel?.length < 1}
          // disabled={!editable}
          disabled={!editable || !job?.level}
          loading={jobRoleLoading}
        />
      </td>
