...
import "../createRoleAndPermissions/style.css";
...
import { SUPER_ADMIN_USER_MANAGEMENT_ROLE_AND_PERMISSION_LIST_PAGE } from "../../../../../config/constants/routePathConstants/superAdmin";
const initialValues = {
  userRoleName: "",
};
const AssignPermissions = () => {
  ...
  const { editRoleAndFeature = {} } = useSelector(authSelector);
  const [loading, setLoading] = useState(false);
  const [errors, setErrors] = useState({});

  const getFeaturesList = () => {
    setLoading(true);
    dispatch(getSingleRoleApi(setLoading, setFormValues, roleId));
  };
  useEffect(() => {
    getFeaturesList();
  }, []);
  const newFilteredFeaturedList = editRoleAndFeature?.features?.map((feature)=>{
    if(feature?.featureName === "Verification Tab"){
      const result = feature?.subFeatures?.filter((subFeature)=>subFeature?.subFeatureName === "Verification Details")
      return {...feature,subFeatures:result}
    }
    return feature
  })
  useEffect(() => {
    setFeatureList(newFilteredFeaturedList);
  }, [editRoleAndFeature]);
    ...
    <div className="main-content">
      {loading ? (
        ...
        <>
          ...
          <section className="sub-admin-wrapper" style={{ display: "none" }}>
                                ...
                                >
                                  <td key={subFeatureIndex}>
                                    <div className="text-check">
                                      ...
                                      />
                                      <label
                                        htmlFor={subFeature?.subFeatureName}
                                      >
