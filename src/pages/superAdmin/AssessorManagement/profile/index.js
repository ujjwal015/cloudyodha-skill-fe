...
const AssessorManagementProfile = () => {
  ...
  const handleSubmit = (event) => {
    ...
    if (Object.keys(formErrors).length === 0) {
      ...
      // dispatch(
      //   createAssessorProfileApi(formDataNew, setLoading, clearFormValues, navigate)
      // );
      sendData(formDataNew);
    } else {
    ...
    <div className="main-content">
      ...
      <Box sx={{ width: "100%" }}>
        ...
        <AssessorTabPanel tabValue={tabValue} index={3}>
          <BankDetail schemeType={schemeType} handleClearScheme={handleClearScheme} />
        </AssessorTabPanel>
      </Box>
    </div>
  );
