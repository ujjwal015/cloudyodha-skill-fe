...
import JobRoleViewModalNCVET from "../../../../../components/common/Modal/JobRoleModalNCVET";
...
const ClientBasedJobRoleTable = () => {
  ...
  const handleJobRoleClick = (event, data) => {
    ...
    if (data.jobRoleByClient.length > 0) {
      setjobRoleData(data.jobRoleByClient);
    ...
    return [
      ...
      },
      {
        ...
        selector: (row) => {
          ...
          return (
            <div className="jobrole-chip" onClick={(e) => handleJobRoleClick(e, row)}>
              JobRole ({jobRoleCount > 0 ? jobRoleCount : "0"})
            </div>
          );
        },
      },
